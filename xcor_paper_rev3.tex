...
We have shown that foregrounds pose two significant challenges for 2D intensity mapping correlation experiments: (1) the largely uncorrelated radio and infrared foreground residuals contribute a sample variance noise to the cross spectrum; and (2) simple geometric effects result in percent-level correlations between fluxes in these two bands, even if their luminosities are uncorrelated. 

important points of this paper
- there are FG flux correlations

- can match CIBER performance from the ground, can probably do better with higher res surveys (eg, DES, happening now)

- first limits on the EOR 21cm-Ly-alpha xspec

- first simulations of 2D vs 3D spectra
2D experiments probe much of the same science on long wavelength scales...i.e., the ones where the anticorrelation is strongest

what does it take to reach the EOR signal

2D vs 3D experiments refrain, and optimism for the future
