...
\shorttitle{21cm--Infrared cross correlation experiments}
...
ATLAS is a 0.5\,m (f/2.0) telescope \citep{tonry11} in Hawaii designed to perform a wide field sky survey for near earth asteroids. The detector is a 10.5k $\times$10.5k CCD array (STA-1600), with a pixel scale of 1.86'', with an overall field of view of $5.5^\circ$. We observe in the I band centered spanning $\lambda=$731--881\,nm, corresponding to $z=$5.1--6.3. While this redshift range doesn't exactly match that of our r radio observations, it overlaps sufficiently for our purpose of characterizing the effects of noise foregrounds in 21cm--IR cross correlation experiments.

\subsection{Correlation of resolved foreground sources}
...
\begin{figure}[h]
\includegraphics[width=6.8in]{images/ir_hists.pdf}
\caption{5' pixels}
\label{fig:irhists}
\end{figure}
...
\begin{figure}[h]
\includegraphics[width=6.8in]{images/ir_images.pdf}
\caption{5' pixels}
\label{fig:irimages}
\end{figure}
...
\begin{figure}[h]
\includegraphics[width=6.8in]{images/cor_grid.pdf}
\caption{aoeuaoeuaoeu}
\label{fig:corgrid}
\end{figure}

\section{Correlation of unresolved foreground sources}

We consider here the 3D cross correlation experiment in order to quantify the advantages and disadvantages from the point of view of foregrounds and noise, in comparison to the 2D experiment considered above, and discuss what sort of experiment is needed for its detection. 

We first build intuition by asking how much of an overlap there is in the fourier space probed by 21\,cm and IR cubes from existing and planned experiments. 

Let's first compare the informa 21\,cm and IR intensity mapping experiments to have overlapping 

what we've learned:
moon
stacking to mitigate airglow flucts
per-night flat fielding to mitigate amp-region errors
current limits % https://www.evernote.com/shard/s316/nl/2147483647/d4684946-3f99-4458-9f8d-280a0951b921/
MWA model averaging % https://www.evernote.com/shard/s316/nl/2147483647/bd9fe7eb-69a8-4b57-8e3a-15389f86da44/
 IR poly subtraction
 
 todo:
 deeper, non-dithered observations, without moon
 repeated on two consecutive nights to compute cross correlation vs ell
 this FG residual level is input to 2D sensitivity study


\section{2D sensitivity study}
done:
still waiting on Marta's model cubes
2D thermal and photon shot noise calculations
relation between kperp and ell

todo:

\section{3D sensitivity study}

what is our general strategy here
why not use silva cubes/results
assume perfect correlation

2D k space overlap

MWA sensitivity math
IR shot noise math

cross and auto spectrum sensitivity results, with and without wedge

\begin{figure}[h]
\includegraphics[width=6.25in]{images/kspace_overlap.pdf}
\caption{aoeuaoeuaoeu}
\label{fig:corgrid}
\end{figure}

\begin{figure}[h]
\includegraphics[width=7in]{images/cross_and_auto_sensitivity.pdf}
\caption{aoeuaoeuaoeu}
\label{fig:corgrid}
\end{figure}
...
3D thermal and photon shot noise calculations
auto spectrum sensitivities
xspec sensitivity % https://www.evernote.com/shard/s316/nl/2147483647/9dbae53e-0733-4a9d-80ba-5f6bf3136694/
opt/pess
wedgecut
cf adam's sensitivity study % https://www.evernote.com/shard/s316/nl/2147483647/d29aae66-7877-438a-be8a-73f6b82b6282/

todo:
still waiting on Marta's model cubes
evaluate SNRs for current/proposed expts % https://www.evernote.com/shard/s316/nl/2147483647/fff4c996-31a3-4b2f-8fb1-4e2a1d164e11/
...
\section{Power spectra of foreground residuals}

introduction:
	brief reminder of why need to do an analysis of foreground auto power spectra,
	they go into the sensitivity analysis
	they tell us about foreground subtraction and masking
...
Similarly, over narrow fields of view, the spherical power spectrum $C_\ell$ of an image $I(\vec{\theta})$ can be shown to be approximately
\begin{equation}
\label{eqn:Cldef0}
...
\end{equation}
and $\Omega$ is the survey solid angle and $d\Omega$ is the pixel size. Thus, we need only evaluate a fourier transform in order to estimate $C_\ell$ over a narrow field of view. Writing this out in detail, we find 
\begin{equation}
\label{eqn:Cldef}
	C(\ell(a,b))=\left\langle\left|\sum_{m,n}I(m,n)\exp\left(-\frac{2\pi i}{N}  (am+bn)\right)\right|^2\right\rangle\frac{d\theta^2}{N^2}
\end{equation}
where $d\theta^2$ is the pixel solid angle, $N\equiv N_x=N_y$ is number of pixels on a side of a square image, and $\ell=\sqrt{\ell_x^2+\ell_y^2}$, where 
\begin{eqnarray}
...
\end{equation}
Note that $\ell$ has units of 1/rad, so $\Delta_\ell$ has the same units as I. In Appendix \ref{sec:pspecrelation} we derive a relation between the 3D power spectrum and the spherical angular power spectrum which we make use of later in this section.
...
I band overlaps with the EOR0 high band, but doesn't match exactly
but that's okay because we are primarily concerned with characterizing the effects of foregrounds in this paper
...
Having quantified realistic levels of foreground residuals in current 21\,cm and IR observations, we assess the sensitivity of current experiments to the cosmological cross correlation, and examine what improvements are necessary for unambiguous detection. We first consider the cross spectrum and its renormalized cousin, the cross correlation, as possible statistics to quantify the relation between the 21\,cm and IR fields, and argue for focusing on the cross spectrum in the near term.

\subsection{Cross spectrum vs coherence}

We first extend the definition of the angular power spectrum given in Eqn. \ref{eqn:Cldef0} to the cross spectrum case as
\begin{equation}
\label{eqn:Cldefcross}
	C_{A,B}(\vec{\ell}) = \frac{\langle \tilde{I}_A^*(\vec{\ell})\tilde{I}_B(\vec{\ell})\rangle}{\Omega}
\end{equation}
where $A$ and $B$ denote the 21\,cm and the IR fields. The cross spectrum is a quantity which ranges from $-\sqrt{C_{A}(\vec{\ell})C_{B}(\vec{\ell})}$ to $\sqrt{C_{A}(\vec{\ell})C_{B}(\vec{\ell})}$, depending on how correlated or anti-correlated the two fields are. 

The cross spectrum is thus often computed with a different normalization
\begin{equation}
\label{eqn:Cldefcross}
	\rho_{A,B}(\vec{\ell}) = \frac{C_{A,B}(\vec{\ell}) }{\sqrt{C_A(\vec{\ell})  C_B(\vec{\ell}) }}
\end{equation}
which equals -1, 0, on 1 in the cases of perfect anti-correlation, zero correlation, or perfect correlation, respectively. We refer to this re-normalized cross correlation as the coherence. The coherence has the advantage of quantifying the percent correlation between the two fields, that is, it is insensitive to a simple rescaling of either field. However, the large foreground residuals afflicting present observations afflicting both auto power spectra will substantially bias the coherence towards zero. 

Another way to say this is that the error bars on the coherence are larger than those on the cross spectrum because we must subtract from each auto power spectrum in the denominator an estimate of the foreground power spectrum \citep{lidz09,furlanettolidz07}. The only physical information likely to be robust against bias is its sign. Given this, we we opt to simply work with the cross spectrum itself from which the same sign information is available.
...
so there are several ways to increase SNR
larger field of view
higher 21cm resolution
better 21cm FG subtraction
better IR FG subtraction
